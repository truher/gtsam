...
struct GTSAM_EXPORT PreintegratedPlanarRotationParams {
  ...
  virtual bool equals(const PreintegratedPlanarRotationParams& other, double tol=1e-9) const;
...
class GTSAM_EXPORT PreintegratedPlanarRotation {
 ...
 protected:
  ...
  explicit PreintegratedPlanarRotation(const std::shared_ptr<Params>& p) : p_(p) {
    resetIntegration();
  }
...
template <>
struct traits<PreintegratedPlanarRotation> : public Testable<PreintegratedPlanarRotation> {};
